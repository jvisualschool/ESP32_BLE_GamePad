...
AXS15231B 디스플레이 드라이버와 **BLE HID**(Bluetooth Low Energy Human Interface Device) 게임패드(IINE L1161)를 통합하여, 터치/게임패드 양쪽으로 테마를 전환할 수 있는 데모 프로젝트입니다.
...
1. **BLE Central 스캔** — `BLEScan`으로 주변 기기 탐색, 타겟 이름("IINE L1161") 매칭
...
```
[ESP32-S3 Central]                    [IINE L1161 Peripheral]
       |                                       |
       |── BLE Scan (Active, 10s) ────────────>|
       |                                       |
       |<── Advertisement ("IINE L1161") ─────|
       |                                       |
...
- **BLE 게임패드 발견**: IINE L1161 스캔 및 기본 연결 구현
